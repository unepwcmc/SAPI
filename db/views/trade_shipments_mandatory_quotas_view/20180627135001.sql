    ...
    (
      SELECT ts.id AS id,ts.year AS year,ts.appendix AS appendix,ts.taxon_concept_full_name AS taxon,ts.taxon_concept_class_name AS class,ts.taxon_concept_order_name AS order,ts.taxon_concept_family_name AS family,ts.taxon_concept_genus_name AS genus,CASE WHEN ts.reported_by_exporter THEN ts.quantity ELSE NULL END AS exporter_quantity,CASE WHEN ts.reported_by_exporter THEN NULL ELSE ts.quantity END AS importer_quantity,unit.name_en AS unit,importer.iso_code2 AS importer_iso,importer.name_en AS importer,exporter.iso_code2 AS exporter_iso,exporter.name_en AS exporter,NULL AS origin,purpose.name_en AS purpose,source.name_en AS source,term.name_en AS term,ts.import_permits_ids AS import_permits,ts.export_permits_ids AS export_permits,ts.origin_permits_ids AS origin_permits,ts.import_permit_number AS import_permit,ts.export_permit_number AS export_permit,ts.origin_permit_number AS origin_permit,'Quota' AS issue_type,'CoP_approved' AS details_of_compliance_issue,'09/06/1997' AS compliance_type_start_date,'Present' AS compliance_type_end_date,'Capra falconeri' AS compliance_type_taxon,'SPECIES' AS compliance_type_taxonomic_rank,'12' AS quota_quantity,'hunting trophies  Conf. 10.15 (Rev. CoP14)' AS notes
