WITH synonyms_segmented(taxon_concept_id, full_name, author_year, matched_taxon_concept_id, matched_name, matched_name_segment) AS (
  SELECT
    ...
    tc.id,
    tc.full_name,
    UPPER(REGEXP_SPLIT_TO_TABLE(tc.full_name, '(?:(?!-)[[:space:][:punct:]])+'))
  ...
  SELECT
    ...
    id,
    taxon_concepts.full_name,
    UPPER(REGEXP_SPLIT_TO_TABLE(full_name, '(?:(?!-)[[:space:][:punct:]])+'))
  FROM taxon_concepts
  ...
  SELECT
    ...
    taxon_concepts.id,
    taxon_concepts.full_name,
    UPPER(REGEXP_SPLIT_TO_TABLE(taxon_concepts.full_name, '(?:(?!-)[[:space:][:punct:]])+'))
  FROM taxon_concepts
  JOIN ranks
  ...
  WHERE taxon_concepts.name_status NOT IN ('S', 'T', 'N')
    ...
    AND CASE
      ...
      ELSE
        (taxon_concepts.listing->'cites_historically_listed')::BOOLEAN
        OR
        (taxon_concepts.listing->'eu_historically_listed')::BOOLEAN
    END
  ...
  SELECT
    ...
    UPPER(REGEXP_SPLIT_TO_TABLE(taxon_common_names.name, '(?:(?!-)[[:space:][:punct:]])+'))
  ...
  SELECT * FROM (
    SELECT
      taxon_concept_id, full_name, author_year,
      matched_taxon_concept_id, matched_name,
    CASE
      WHEN
        POSITION(
          UPPER(unaccent('unaccent', matched_name_segment))
          IN
          UPPER(unaccent('unaccent', matched_name))
        ) = 1
      THEN UPPER(unaccent('unaccent', matched_name))
      ELSE matched_name_segment
    END AS matched_name_segment,
    type_of_match
    FROM (
      SELECT *, 'SELF' AS type_of_match FROM scientific_names_segmented
      UNION
      SELECT *, 'SYNONYM' FROM synonyms_segmented
      UNION
      SELECT *, 'SUBSPECIES' FROM unlisted_subspecies_segmented
      UNION
      SELECT *, 'COMMON_NAME' FROM common_names_segmented_dehyphenated
    ) all_names_segmented
  ) all_names_segmented_no_prefixes
  WHERE
    LENGTH(matched_name_segment) >= 3
    OR (
      -- Contains non-latin letters
      type_of_match = 'COMMON_NAME' AND matched_name_segment ~ '[^\x00-\u0530]'
    )
...
SELECT
  t1.*,
  UPPER(unaccent('unaccent', matched_name_segment)) AS name_for_matching,
