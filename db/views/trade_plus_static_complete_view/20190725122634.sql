SELECT DISTINCT *
FROM (
  SELECT ts.id, ts.year, ts.appendix, ts.taxon_id,
  ...
  INNER JOIN trade_codes sources ON ts.source = sources.code
  INNER JOIN trade_codes purposes ON ts.purpose = purposes.code
