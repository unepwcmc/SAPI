...
FROM (
  ...
  FROM (
    ...
    FROM (
      ...
      FROM taxon_concept_references tc_refs_1
        JOIN taxon_concepts_and_ancestors_mview tc ON tc_refs_1.is_standard
        AND tc_refs_1.is_cascaded
        AND tc.ancestor_taxon_concept_id = tc_refs_1.taxon_concept_id
    ) cascaded_tc_refs
  ...
  ) cascaded_tc_refs_without_exclusions
UNION ALL
