...
LEFT OUTER JOIN unnested_rules_with_trade_codes ur ON (
  ...
  AND
  (ur.unit_code IS NULL OR (ur.unit_code = 'NULL' AND (ts.unit_id IS NULL)) OR ur.unit_id = COALESCE(tr.output_unit_id, ts.unit_id))
  AND
  (ur.term_code IS NULL OR (ur.term_code = 'NULL' AND ts.term_id IS NULL) OR ur.term_id = COALESCE(tr.output_term_id, ts.term_id))
  AND (
...
LEFT OUTER JOIN unnested_rules_with_trade_codes tur ON (
  ...
  AND
  (tur.unit_code IS NULL OR (tur.unit_code = 'NULL' AND ts.unit_id IS NULL) OR tur.unit_id = COALESCE(ur.output_unit_id, tr.output_unit_id, ts.unit_id))
  AND
  (tur.term_code IS NULL OR (tur.term_code = 'NULL' AND ts.term_id IS NULL) OR tur.term_id = COALESCE(ur.output_term_id, tr.output_term_id, ts.term_id))
  AND (
