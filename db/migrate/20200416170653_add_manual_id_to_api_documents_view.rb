class AddManualIdToApiDocumentsView < ActiveRecord::Migration[4.2]
  def up
    execute "DROP MATERIALIZED VIEW IF EXISTS api_documents_mview"
    execute "DROP VIEW IF EXISTS api_documents_view"

    execute "CREATE VIEW api_documents_view AS #{view_sql('20200416170653', 'api_documents_view')}"
  ...
  def down
    ...
    execute "CREATE VIEW api_documents_view AS #{view_sql('20200228113253', 'api_documents_view')}"
    execute "CREATE MATERIALIZED VIEW api_documents_mview AS SELECT * FROM api_documents_view"
