...
describe Trade::SandboxShipmentsController do
  ...
  describe "PUT update" do
      before(:each) do
        ...
        sandbox_klass.create(:taxon_name => 'Acipenser baerii')
      end
    it "should return success when taxon_name not set" do
      s = sandbox_klass.find_by_taxon_name('Acipenser baerii')
      put :update, :annual_report_upload_id => annual_report_upload.id, 
        :id => s.id, :sandbox_shipment => {:taxon_name => nil, :accepted_taxon_name => nil}, format: :json
      response.body.should be_blank
    end
    it "should return success when taxon_name does not exist" do
      s = sandbox_klass.find_by_taxon_name('Acipenser baerii')
      put :update, :annual_report_upload_id => annual_report_upload.id, 
        :id => s.id, :sandbox_shipment => {:taxon_name => 'Acipenser foobarus'}, 
        :format => :json
      response.body.should be_blank
    end
  end
