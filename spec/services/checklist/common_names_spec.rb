...
describe Checklist do
  ...
  context 'when common names displayed' do
    ...
    it "returns all English names for Arctocephalus australis: 'South American Fur Seal, Southern Fur Seal'" do
      expect(@australis.english_names).to eq([ 'South American Fur Seal', 'Southern Fur Seal' ])
    end

    it "returns all Spanish names for Arctocephalus australis: 'Lobo fino sudamericano, Oso marino austral'" do
      expect(@australis.spanish_names).to eq([ 'Lobo fino sudamericano', 'Oso marino austral' ])
    end

    it "returns all French names for Arctocephalus australis: 'Otarie à fourrure australe'" do
      expect(@australis.french_names).to eq([ 'Otarie à fourrure australe' ])
    end

    it "returns all English names for Arctocephalus spp.: 'Fur seals, Southern fur seals'" do
      expect(@arctocephalus.english_names).to eq([ 'Fur seals 1', 'Southern fur seals' ])
    end

    it "returns all Spanish names for Arctocephalus spp.: 'Osos marinos'" do
      expect(@arctocephalus.spanish_names).to eq([ 'Osos marinos' ])
    end

    it "returns all French names for Arctocephalus spp.: 'Arctocéphales du sud, Otaries à fourrure, Otaries à fourrure du sud'" do
      expect(@arctocephalus.french_names).to eq([ 'Arctocéphales du sud', 'Otaries à fourrure', 'Otaries à fourrure du sud' ])
    end

    it 'includes a species without any common names defined' do
