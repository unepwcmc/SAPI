...
describe TaxonConcept do
  context 'create' do
    ...
    context 'full_name should have leading and trailing spaces trimmed' do
      let(:tc) do
        build_phylum(
          taxonomy_id: cites_eu.id,
          parent_id: kingdom_tc.id,
      ...
      specify { expect(tc.full_name).to eq('Aa Bb') }
      specify { expect(tc.error_on(:full_name).size).to eq(0) }
    end
    ...
    context 'author_year' do
      ...
      specify 'is not valid with cyrillic characters' do
        ...
        expect(species.error_on(:author_year)).to contain_exactly(
          MESSAGE_SHOULD_BE_PDF_SAFE
        )
      end

      specify 'has leading and trailing spaces trimmed before validation' do
        species.author_year = " Cage (1952)\n"

        expect(species.author_year).to eq('Cage (1952)')
        expect(species.error_on(:author_year).size).to eq(0)
      end
    end
  end
