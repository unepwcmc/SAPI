...
describe Designation do
  ...
  describe :update do
    context "when updating a non-protected name" do
      ...
      specify {
        expect(designation.update_attributes(
    ...
    context "when updating a protected name" do
      specify {
        expect(cites.update_attributes(
          { :name => 'RULES OF INTERGALACTIC TRADE' }
        )).to be_falsey
      }
    end
    context "when updating taxonomy with no dependent objects attached" do
      ...
      specify { expect(designation.update_attributes(:taxonomy_id => taxonomy.id)).to be_truthy }
    end
    context "when updating taxonomy with dependent objects attached" do
      ...
      let(:taxonomy) { create(:taxonomy) }
      specify { expect(designation.update_attributes(:taxonomy_id => taxonomy.id)).to be_falsey }
    end
  end
  describe :destroy do
