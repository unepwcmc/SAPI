class Species::ShowTaxonConceptSerializerCites < Species::ShowTaxonConceptSerializer
  ...
  def processes
    CitesProcess.includes(
      ...
      'resolution DESC', 'geo_entities.name_en'
    )
  end
  ...
  def quotas
    Quota.from(
      'api_cites_quotas_view AS trade_restrictions'
    ).where(
      (
        <<-SQL.squish
      ...
      object_and_children: object_and_children,
      ancestors: ancestors,
      taxon_concept_id: object.id
    ).select(
      <<-SQL.squish
  ...
  def cites_suspensions
    CitesSuspension.from(
      'api_cites_suspensions_view AS trade_restrictions'
    ).where(
      (
        <<-SQL.squish
          trade_restrictions.taxon_concept_id IN (:object_and_children)
  ...
  def cites_listing_changes
    rel = MCitesListingChange.from(
      'api_cites_listing_changes_view AS listing_changes_mview'
    ).where(
  ...
  def eu_listing_changes
    rel = MEuListingChange.from(
      'api_eu_listing_changes_view AS listing_changes_mview'
    ).where(
      'listing_changes_mview.taxon_concept_id' => object_and_children
    )
...
private
  ...
  def force_anthozoa_statement
    ...
    eu_decision_type = EuDecisionType.find_by(name: 'Negative')
