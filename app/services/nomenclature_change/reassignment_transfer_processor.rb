class NomenclatureChange::ReassignmentTransferProcessor < NomenclatureChange::ReassignmentProcessor
  ...
  def transferred_object_before_save(reassignment, reassignable)
    ...
    if reassignment.kind_of?(NomenclatureChange::ParentReassignment) ||
      ...
      if duplicate.blank?
        ...
        reassignable.document_citation_taxon_concepts.find_by(
  ...
  def summary_line
    "The following associations will be transferred from #{
...
protected
  ...
  def transfer_distribution_references(reassigned_object, reassignable)
    ...
    if reassigned_object.distribution_references.count > 0
      distribution_references_to_transfer =
        distribution_references_to_transfer.where.not(
          reference_id: reassigned_object.distribution_references.pluck(
            :reference_id
          )
        )
    end

    distribution_references_to_transfer.update_all(
      distribution_id: reassigned_object.id
    )
  end
  ...
  def transfer_distribution_taggings(reassigned_object, reassignable)
    ...
    taggings_to_transfer = reassignable.taggings.where.not(
      tag_id: reassigned_object.taggings.pluck(:tag_id)
    )

    taggings_to_transfer.update_all(
      taggable_id: reassigned_object.id
    )
  end
  ...
  def transfer_party_listing_distribution(reassigned_object, reassignable)
    ...
    reassignable.party_listing_distribution.update_attribute(
      :listing_change_id, reassigned_object.id
    )
  end
  ...
  def transfer_listing_distributions(reassigned_object, reassignable)
    ...
    if reassigned_object.listing_distributions.count > 0
      listing_distributions_to_transfer =
        listing_distributions_to_transfer.where.not(
          geo_entity_id: reassigned_object.listing_distributions.pluck(
            :geo_entity_id
          )
        )
    end

    listing_distributions_to_transfer.update_all(
      listing_change_id: reassigned_object.id
    )
  end
  ...
  def transfer_taxonomic_exclusions(reassigned_object, reassignable)
    ...
    if reassigned_object.taxonomic_exclusions.count > 0
      taxonomic_exclusions_to_transfer =
        taxonomic_exclusions_to_transfer.where.not(
          taxon_concept_id: reassigned_object.taxonomic_exclusions.pluck(
            :taxon_concept_id
          )
        )
    end

    taxonomic_exclusions_to_transfer.update_all(
      parent_id: reassigned_object.id
    )
  end

  def transfer_geographic_exclusions(reassigned_object, reassignable)
    reassignable.geographic_exclusions.update_all(
      parent_id: reassigned_object.id
    )
  end
end
