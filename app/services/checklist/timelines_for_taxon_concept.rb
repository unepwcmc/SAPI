class Checklist::TimelinesForTaxonConcept
  ...
  def initialize(taxon_concept)
    ...
    @time_start = Time.zone.local(1975, 01, 01)
    @time_end = Time.zone.local(Time.zone.now.year + 2, 1, 1)
    generate_timelines
    ...
    @timeline_years = @time_start.year.step((@time_end.year - (@time_end.year % 5) + 5), 5).
      to_a.map do |year|
        Checklist::TimelineYear.new({
          taxon_concept_id: @taxon_concept_id,
          year: year,
          pos: ((Time.zone.local(year, 1, 1) - @time_start) / (@time_end - @time_start)).round(2)
        })
      end
  end
end
