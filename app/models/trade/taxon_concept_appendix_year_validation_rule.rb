...
class Trade::TaxonConceptAppendixYearValidationRule < Trade::InclusionValidationRule
  ...
  def validation_errors_for_shipment(shipment)
    ...
    appendix_node = v['appendix'].eq(Arel::Nodes::Quoted.new(shipment.appendix))
    ...
    taxon_concept_node = v['taxon_concept_id'].eq(Arel::Nodes::Quoted.new(shipment.taxon_concept_id))
  ...
  def taxon_concept_join_node(s, v)
    s['taxon_concept_id'].eq(Arel::Nodes::Quoted.new(v['taxon_concept_id']))
  end

  def appendix_join_node(s, v)
    s['appendix'].eq(Arel::Nodes::Quoted.new(v['appendix']))
  end
