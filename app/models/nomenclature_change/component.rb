...
class NomenclatureChange::Component < ActiveRecord::Base
  ...
  attr_accessible :created_by_id, :is_input, :is_output,
    :nomenclature_change_id, :taxon_concept_id, :note, :updated_by_id,
    :input_parent_reassignments_attributes,
    :input_name_reassignments_attributes,
    :input_distribution_reassignments_attributes,
    :input_legislation_reassignments_attributes
  belongs_to :taxon_concept
  has_many :input_parent_reassignments,
    :class_name => NomenclatureChange::ParentReassignment,
    :foreign_key => :nomenclature_change_input_id, :dependent => :destroy
  has_many :input_name_reassignments,
    :class_name => NomenclatureChange::NameReassignment,
    :foreign_key => :nomenclature_change_input_id, :dependent => :destroy
  has_many :input_distribution_reassignments,
    :class_name => NomenclatureChange::DistributionReassignment,
    :foreign_key => :nomenclature_change_input_id, :dependent => :destroy
  has_many :input_legislation_reassignments,
  ...
  accepts_nested_attributes_for :input_distribution_reassignments, :allow_destroy => true
  accepts_nested_attributes_for :input_legislation_reassignments, :allow_destroy => true
end
