...
class Quota < TradeRestriction
  ...
  def self.years_array
    self.select('EXTRACT(year from start_date) years').
  ...
  def self.count_matching params
    Quota.where([
        ...
        :excluded_geo_entities => params[:excluded_geo_entities_ids].present? ?
          params[:excluded_geo_entities_ids].join(",") : nil,
        :included_geo_entities => params[:included_geo_entities_ids].present? ?
          params[:included_geo_entities_ids].join(",") : nil,
        :excluded_taxon_concepts => params[:excluded_taxon_concepts_ids].presence,
        :included_taxon_concepts => params[:included_taxon_concepts_ids].presence]).
      count
  end
end
