      ...
      ApplicationRecord.send(
        ...
        [
          ...
          excluded_taxon_concepts_ids:
            if options['excluded_taxon_concepts_ids'].present?
              options['excluded_taxon_concepts_ids'].split(',').map(&:to_i)
            else
              nil
            end,
          included_taxon_concepts_ids:
            if options['included_taxon_concepts_ids'].present?
              options['included_taxon_concepts_ids'].split(',').map(&:to_i)
            else
              nil
            end,
          excluded_geo_entities_ids: (
            options['excluded_geo_entities_ids'] &&
            options['excluded_geo_entities_ids'].map(&:to_i)
          ),
          included_geo_entities_ids: (
            options['included_geo_entities_ids'] &&
