= CITES Checklist {<img src="https://secure.travis-ci.org/unepwcmc/SAPI.png" />}[http://travis-ci.org/unepwcmc/SAPI] {<img src="https://codeclimate.com/badge.png" />}[https://codeclimate.com/github/unepwcmc/SAPI]

==Dependencies

packages as for Ubuntu:

  - libpq-dev postgresql-9.1 postgresql-contrib-9.1
  - freetds-bin freetds-common freetds-dev
  - pdftk

Redis is used for the job queue.
...
=== Data import
  ...
  rake import:all 
This task loads the db seeds, and imports species, distributions, cites listings, common names, synonyms, references and taxon concept references directly from MS SQL (you'll need freetds installed); we're still using files in lib/assets/files/ to import cites_regions, countries and standard references.
There are specific tasks for each of the types of data. Run 'rake -T' to get the list of tasks.

You need to run imports from a machine that has access to both the MSSQL db and PostgreSQL. Port forwarding might come in handy:

  ssh -fNg -L 15432:localhost:5432 ssh_user@public-address-of-db-host
...
Stored procedures are not updated with regular migrations anymore, as that approach was cumbersome. Instead, procedures and other plpgsql constructs are defined in separate files under db/plpgsql and are hooked to the db:migrate task, so that every time db:migrate runs those files are executed in lexicographic order. Therefore, it is important to use 'CREATE OR REPLACE' or 'DROP' / 'CREATE' in these files for migrations to pass.
...
2. Traverse the taxonomic tree from the root down
   Cascade listings from higher taxa to lower, except for cases where a current listing is already present or taxon concept is NC.
...
==== Filtering taxa by their listing information ====
...
* cites_del: if true, taxon concept has been deleted from the appendices
* usr_cites_exclusion: if true, taxn concept is excluded from its parent listing (set by the user)
* cites_exclusion: if true, taxon concept is excluded from its parent listing (calculated from ancestors usr_cites_exclusion flags)
...
1. taxa listed in any of the 3 CITES appendices (both implicitly or explicitly) will have listing->'cites_X' and listing->'cites_listing' set
2. taxa previously listed but then deleted from appendices will have listing->'cites_del' set to true and listing->'cites_listing' blank while listing->'cites_listed' is still set
3. taxa listed as NC (e.g. Aloe vera, Pereskia spp.) will have listing->'usr_cites_exclusion' set to true and listing->'cites_listing' blank
4. taxa that have never been listed will have listing->'cites_listing' blank
