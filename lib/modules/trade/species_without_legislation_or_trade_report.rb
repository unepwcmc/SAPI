class Trade::SpeciesWithoutLegislationOrTradeReport
  ...
  def initialize
    @query = TaxonConcept.from(<<-SQL
      (
        WITH cites_species (
          ...
          full_name, author_year, name_status, cites_listed_descendants, taxonomic_position) AS (
          SELECT taxon_concepts.id, taxon_concepts.legacy_id,
          data->'kingdom_name', data->'phylum_name', data->'class_name', data->'order_name',
          data->'family_name', data->'genus_name', data->'species_name',
          taxon_concepts.full_name, taxon_concepts.author_year, taxon_concepts.name_status, listing -> 'cites_listed_descendants', taxon_concepts.taxonomic_position
          FROM taxon_concepts
          ...
          GROUP BY taxon_concepts.id, legacy_id, kingdom_name, phylum_name, class_name, order_name, family_name, genus_name, species_name, full_name, author_year, name_status, cites_listed_descendants, taxonomic_position
          EXCEPT
          SELECT taxon_concepts.*
    ...
    SQL
      ...
      :full_name, :author_year, :name_status, :cites_listed_descendants,
      'COUNT(quotas.id)',
    ...
    group(:'taxon_concepts.id', :legacy_id,
      ...
      :full_name, :author_year, :name_status, :cites_listed_descendants, :taxonomic_position
    ).
