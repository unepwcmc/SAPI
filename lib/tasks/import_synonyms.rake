        ...
        sql = <<-SQL
          ...
          FROM (
            ...
            INNER JOIN taxon_concepts AS synonym
              ON synonym.author_year = UPPER(BTRIM(#{TMP_TABLE}.author)) AND synonym.parent_id = #{TMP_TABLE}.parent_id AND synonym.data->'accepted_id' = #{TMP_TABLE}.accepted_id::VARCHAR AND synonym.rank_id = synonyms_rank.id AND synonym.legacy_type = '#{kingdom}'
