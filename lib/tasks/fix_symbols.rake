namespace :import do
  ...
  task fix_annotations_symbols: :environment do
    CSV.foreach('lib/files/ascii_symbols_utf8.csv', quote_char: 'h') do |row|
      ...
      Annotation.where('short_note_en LIKE :sym OR full_note_en LIKE :sym', sym: "%#{row[0]}%").find_each do |o|
  ...
  task fix_common_names_symbols: :environment do
    CSV.foreach('lib/files/ascii_symbols_utf8.csv', quote_char: 'h') do |row|
      ...
      puts "#{count} records affected"
      CommonName.where('name Like ?', "%#{row[0]}%").find_each do |o|
  ...
  task fix_taxon_concepts_symbols: :environment do
    CSV.foreach('lib/files/ascii_symbols_utf8.csv', quote_char: 'h') do |row|
      ...
      puts "#{count} records affected"
      TaxonConcept.where('author_year Like ?', "%#{row[0]}%").find_each do |o|
  ...
  task fix_geo_entities_symbols: :environment do
    CSV.foreach('lib/files/ascii_symbols_utf8.csv', quote_char: 'h') do |row|
      ...
      puts "#{count} records affected"
      GeoEntity.where('name_en Like ?', "%#{row[0]}%").find_each do |o|
  ...
  task fix_references_symbols: :environment do
    CSV.foreach('lib/files/ascii_symbols_utf8.csv', quote_char: 'h') do |row|
      ...
      puts "#{count} records affected"
      Reference.where('citation Like ?', "%#{row[0]}%").find_each do |o|
  ...
  task add_italics_to_references: :environment do
    ...
    Reference.where('citation like ? ', '_%_').find_each do |o|
