...
def import_data_for_kew_id(kingdom, rank, synonyms = nil)
  ...
  if synonyms
    ...
    ApplicationRecord.connection.execute('CREATE INDEX synonym_import_name ON synonym_kew_id_import (name)')
  else
    ApplicationRecord.connection.execute('DROP INDEX IF EXISTS species_import_name')
    ApplicationRecord.connection.execute('CREATE INDEX species_import_name ON species_kew_id_import (name)')
  end
