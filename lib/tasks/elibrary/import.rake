...
namespace :elibrary do
  ...
  namespace :non_cites_taxa do
    ...
    task import: :environment do |task_name|
      ...
      importer = Elibrary::NonCitesTaxaImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  ...
  namespace :events do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::EventsImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :documents do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::DocumentsImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :identification_documents do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::DocumentsIdentificationImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::DocumentDiscussionsImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
    end
  ...
  namespace :users do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::UsersImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations_cop do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsCopImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations_rst do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsRstImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations_ndf do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsNdfImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations_no_event do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsNoEventImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
  namespace :citations_manual do
    ...
    task import: :environment do |task_name|
      check_file_provided(task_name)
      importer = Elibrary::CitationsManualImporter.new(ENV.fetch('FILE', nil))
      importer.run
    end
  end
